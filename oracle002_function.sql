...
/*-------------------------------------------------------------
일반함수(NULL)
2. nvl2(컬럼, 대체값1, 대체값2) :컬럼의 값이 NOT null 이면 대체값1, 
                                null이면 대체값2로 출력한다.
---------------------------------------------------------------*/
...
--'AC+MGR'인 사원은 15%, 'PU_CLERK'인 사원은 20%를 인상
SELECT job_id, salary,
        decode(job_id, 'PR_REP', salary * 1.05,
                       'SA_MAN', salary * 1.10,
                       'AC+MGR', salary * 1.15,
                       'PU_CLERK', salary * 1.20, salary) AS newsal
FROM employees;
...
SELECT salary, CASE WHEN salary < 10000 THEN '하'
...
sum(DISTINCT salary), avg(DISTINCT salary), stddev(DISTINCT salary), 
variance(DISTINCT salary)
FROM employees;
...
-- 부서별 최소급여, 최대급여를 출력하는데 최소급여와 최대급여가 같지 않은 경우에만 부서별 오름차순으로
-- 출력
